...
(function(){
  ...
  function convertEditor(el){if(!el||el.querySelector('.i-type')?.value!=='Crop Top')return;const box=el.querySelector('.measurement-editor');if(box&&!box.querySelector('.crop-top-four-measure'))box.innerHTML=fields()}
  function renderEditors(){document.querySelectorAll('.item-editor').forEach(convertEditor)}
  document.addEventListener('change',e=>{if(e.target.classList?.contains('i-type'))setTimeout(()=>convertEditor(e.target.closest('.item-editor')),0)},false);
  ...
  function reorderCropTopCard(card){
    if(!card||card.dataset.cropTopLayoutFixed==='1'||!/Crop Top/i.test(card.textContent||''))return;
    const leaf=[...card.querySelectorAll('*')].filter(el=>el.children.length===0);
    const noteText=leaf.find(el=>el.textContent.trim()==='IMPORTANT NOTE');
    const measureTitle=leaf.find(el=>el.textContent.trim()==='MEASUREMENTS');
    const measureView=card.querySelector('.four-measure-view');
    if(!noteText||!measureTitle||!measureView)return;
    const noteBox=noteText.parentElement;
    const titleBox=measureTitle.parentElement;
    const viewBox=measureView.parentElement;
    if(!noteBox||!titleBox||!viewBox)return;
    const parent=noteBox.parentElement;
    if(!parent)return;
    parent.insertBefore(titleBox,noteBox.nextSibling);
    parent.insertBefore(viewBox,titleBox.nextSibling);
    card.dataset.cropTopLayoutFixed='1';
  }
  function fixCards(){document.querySelectorAll('.job.tailor-job,.job').forEach(reorderCropTopCard)}
  const mo=new MutationObserver(()=>{renderEditors();fixCards()});mo.observe(document.body,{childList:true,subtree:true});
  setTimeout(()=>{renderEditors();fixCards()},0);
})();
