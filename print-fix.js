(function(){
  ...
  function formatDate(v){const s=String(v||'');const m=s.match(/^(\d{4})-(\d{2})-(\d{2})$/);return m?`${m[3]}/${m[2]}/${m[1]}`:s||'—';}
  ...
  function build(no,items,bill,customer,source){
    ...
    let h='<!doctype html><html><head><meta charset="utf-8"><title>Fashion Factory Labels</title><style>'+
      ...
      '.label{flex:0 0 36mm;width:36mm;height:36mm;padding:1.55mm 1.2mm .7mm;overflow:hidden;display:flex;flex-direction:column;align-items:center;background:#fff}'+
      ...
      '.info{font-size:7.3px;line-height:1.05;font-weight:900;text-align:center;text-transform:uppercase;margin-bottom:.35mm;white-space:nowrap}.item-info{font-size:7.2px;line-height:1.05;font-weight:900;text-align:center;text-transform:uppercase;margin-bottom:.3mm;white-space:nowrap;max-width:100%;overflow:hidden;text-overflow:ellipsis}.qty{font-size:9.5px;line-height:1;font-weight:900;text-align:center;text-transform:uppercase;margin:.1mm 0 .35mm;white-space:nowrap}'+
      ...
      '.item-name{font-size:7px;line-height:1.05;font-weight:900;text-transform:uppercase;text-align:center;margin-bottom:.4mm;max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.product-label .item-name{font-size:7.2px;margin-top:.1mm;margin-bottom:.45mm}'+
      ...
      '.jeans-row{width:32.2mm;display:flex;gap:1.2mm;justify-content:center;margin-top:.5mm}.jeans-box{width:15.5mm;height:13mm;border:1px solid #000;border-radius:1.4mm;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:.5mm}.jeans-title{font-size:4.8px;line-height:1;font-weight:900;text-transform:uppercase;margin-bottom:.8mm}.jeans-value{font-size:10px;line-height:1;font-weight:900}.other{width:32mm;border:1px solid #000;border-radius:1.4mm;padding:1.4mm;font-size:6px;line-height:1.1;font-weight:900;text-align:center;margin-top:.5mm;max-height:12mm;overflow:hidden}.date{font-size:6.3px;line-height:1;font-weight:900;text-align:center;text-transform:uppercase;margin-top:auto;padding-top:.45mm;white-space:nowrap}.product-label .brand{margin-bottom:.7mm}.product-label .customer{font-size:8.2px;margin-bottom:.55mm}.product-label .bill-value{margin-bottom:.45mm}.product-label .measure-wrap{margin-top:.35mm}'+
      '</style></head><body>';
    rows.forEach(({s})=>{
      const type=s.item_type||'',isJeans=type==='Jeans',isOther=type==='Other Item';
      const f1=s.measurements?.f1||'—',f2=s.measurements?.f2||'—',s1=s.measurements?.s1||'—',s2=s.measurements?.s2||'—',length=s.measurements?.length||'—',waist=s.measurements?.waist||'—',billValue=bill||'—',customerValue=customer||'—',qtyValue=s.quantity||1,dateValue=formatDate(s.delivery_date);
      h+='<div class="row">';
      h+='<div class="label bag-label"><div class="brand">FASHION FACTORY</div><div class="customer">'+esc(customerValue)+'</div><div class="bill-label">BILL NO.</div><div class="bill-value">'+esc(billValue)+'</div><div class="item-info">ITEM: '+esc(type||'—')+'</div><div class="info">DELIVERY: '+esc(dateValue)+'</div><div class="qty">QTY: '+esc(qtyValue)+'</div>'+barcodeSvg(billValue)+'<div class="bag-human">'+esc(billValue)+'</div></div>';
      h+='<div class="label product-label"><div class="brand">FASHION FACTORY</div><div class="customer">'+esc(customerValue)+'</div><div class="bill-label">BILL NO.</div><div class="bill-value">'+esc(billValue)+'</div><div class="item-name">'+esc(type||'—')+' QTY: '+esc(qtyValue)+'</div>';
      ...
      h+='<div class="date">DELIVERY: '+esc(dateValue)+'</div></div></div>';
    });
