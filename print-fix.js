(function(){
  ...
  function build(no,items,bill,customer,source){
    ...
    let h='<!doctype html><html><head><meta charset="utf-8"><title>Fashion Factory Labels</title><style>'+
      ...
      '.bill-label{font-size:6px;line-height:1;font-weight:900;text-align:center;text-transform:uppercase;margin-bottom:.15mm}'+
      '.bill-value{font-size:8.8px;line-height:1;font-weight:900;text-align:center;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;margin-bottom:.55mm}'+
      '.info{font-size:6.2px;line-height:1;font-weight:900;text-align:center;text-transform:uppercase;margin-bottom:.4mm;white-space:nowrap}'+
      '.qty{font-size:8.3px;line-height:1;font-weight:900;text-align:center;text-transform:uppercase;margin:.1mm 0 .35mm;white-space:nowrap}'+
      '.bag-barcode{display:block;width:32mm;height:11.2mm;margin:auto 0 .1mm}'+
      '.bag-human{font-size:5.8px;line-height:1;font-weight:900;text-align:center;margin-top:.15mm;text-transform:uppercase;max-width:32mm;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}'+
      ...
      '.side-letter{width:5.8mm;display:flex;align-items:center;justify-content:center;font-size:13.5px;line-height:1;font-weight:900}'+
      '.measure-col{width:8.1mm;display:flex;flex-direction:column;align-items:center;justify-content:center}'+
      '.measure-value{font-size:10.8px;line-height:1;font-weight:900}'+
      '.measure-line{width:6.8mm;border-top:1px solid #000;margin:.72mm 0}'+
      '.measure-divider{height:100%;border-left:1px solid #000}'+
      '.jeans-row{width:32.2mm;display:flex;gap:1.2mm;justify-content:center;margin-top:.5mm}'+
      '.jeans-box{width:15.5mm;height:13mm;border:1px solid #000;border-radius:1.4mm;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:.5mm}'+
      ...
      '.other{width:32mm;border:1px solid #000;border-radius:1.4mm;padding:1.4mm;font-size:6px;line-height:1.1;font-weight:900;text-align:center;margin-top:.5mm;max-height:12mm;overflow:hidden}'+
      '.date{font-size:6.3px;line-height:1;font-weight:900;text-align:center;text-transform:uppercase;margin-top:auto;padding-top:.45mm;white-space:nowrap}'+
      '.product-label .brand{margin-bottom:.7mm}'+
      '.product-label .customer{font-size:8.2px;margin-bottom:.55mm}'+
      '.product-label .bill-value{margin-bottom:.45mm}'+
      '.product-label .item-name{margin-bottom:.25mm}'+
      '.product-label .qty{font-size:8.3px;margin-bottom:.05mm}'+
      '.product-label .measure-wrap{margin-top:.35mm}'+
      '</style></head><body>';

    rows.forEach(({s})=>{
      const type=s.item_type||'';
      const isJeans=type==='Jeans';
      const isOther=type==='Other Item';
      const f1=s.measurements?.f1||'—',f2=s.measurements?.f2||'—';
      const s1=s.measurements?.s1||'—',s2=s.measurements?.s2||'—';
      const length=s.measurements?.length||'—',waist=s.measurements?.waist||'—';
      const billValue=bill||'—',customerValue=customer||'—';

      h+='<div class="row">';

      // LEFT: BAG LABEL — customer, bill, delivery, qty, large bill barcode.
      h+='<div class="label bag-label">'+
      ...
      // RIGHT: PRODUCT LABEL — no barcode, F left / S right, delivery at bottom.
      h+='<div class="label product-label">'+
        '<div class="brand">FASHION FACTORY</div>'+
        '<div class="customer">'+esc(customerValue)+'</div>'+
        '<div class="bill-label">BILL NO.</div>'+
        '<div class="bill-value">'+esc(billValue)+'</div>'+
        '<div class="item-name">'+esc(type||'—')+' • QTY '+esc(s.quantity||1)+'</div>';

      if(isJeans){
        h+='<div class="jeans-row">'+
          '<div class="jeans-box"><div class="jeans-title">LENGTH CUTTING</div><div class="jeans-value">'+esc(length)+'</div></div>'+ 
          '<div class="jeans-box"><div class="jeans-title">KAMAR FITTING</div><div class="jeans-value">'+esc(waist)+'</div></div>'+ 
        '</div>';
      }else if(isOther){
        h+='<div class="other">'+esc(s.alteration_details||'—')+'</div>';
      }else{
        // F on LEFT, S on RIGHT; each side has UP / DOWN values.
  ...
  window.printLabels=function(no,items,bill,customer,source){
    ...
    w.document.write(build(no,items,bill,customer,source));
    w.document.close();
