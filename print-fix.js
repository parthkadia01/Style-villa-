(function(){
  ...
  function build(no,items,bill,customer,source){
    ...
    let h='<!doctype html><html><head><meta charset="utf-8"><title>Fashion Factory Labels</title><style>'+
      ...
      '.info{font-size:7.3px;line-height:1.05;font-weight:900;text-align:center;text-transform:uppercase;margin-bottom:.45mm;white-space:nowrap}.qty{font-size:9.5px;line-height:1;font-weight:900;text-align:center;text-transform:uppercase;margin:.1mm 0 .35mm;white-space:nowrap}'+
    ...
    rows.forEach(({s})=>{
      ...
      const f1=s.measurements?.f1||'—',f2=s.measurements?.f2||'—',s1=s.measurements?.s1||'—',s2=s.measurements?.s2||'—',length=s.measurements?.length||'—',waist=s.measurements?.waist||'—',billValue=bill||'—',customerValue=customer||'—',qtyValue=s.quantity||1;
      h+='<div class="row">';
      h+='<div class="label bag-label"><div class="brand">FASHION FACTORY</div><div class="customer">'+esc(customerValue)+'</div><div class="bill-label">BILL NO.</div><div class="bill-value">'+esc(billValue)+'</div><div class="info">DELIVERY: '+esc(s.delivery_date||'—')+'</div><div class="qty">QTY: '+esc(qtyValue)+'</div>'+barcodeSvg(billValue)+'<div class="bag-human">'+esc(billValue)+'</div></div>';
