(function(){
  ...
  function build(no,items,bill,customer,source){
    ...
    let h='<!doctype html><html><head><meta charset="utf-8"><title>Fashion Factory Labels</title><style>'+
      ...
      'html,body{margin:0;padding:0;width:78mm;height:36mm;background:#fff;font-family:Arial,sans-serif}'+
    ...
    rows.forEach(({it,s,i,detail})=>{
      h+='<div class="row">';
      h+='<div class="label"><div class="brand">FASHION FACTORY</div><div class="txt"><b>Bill:</b> '+esc(bill||'—')+'<br><b>Customer:</b> '+esc(customer||'—')+'<br><b>Delivery:</b> '+esc(s.delivery_date||'—')+'<br><b>Qty:</b> '+esc(s.quantity||1)+'</div><svg class="barcode" id="b'+i+'"></svg><div class="code">BAG • '+esc(it.bag_barcode||'')+'</div></div>';
      h+='<div class="label"><div class="brand">FASHION FACTORY</div><div class="txt"><b>Bill:</b> '+esc(bill||'—')+'<br><b>Item:</b> '+esc(s.item_type||'—')+'</div><div class="detail">'+esc(detail)+'</div><svg class="barcode" id="p'+i+'"></svg><div class="code">PRODUCT • '+esc(it.product_barcode||'')+'</div></div>';
      h+='</div>';
    });
  ...
  window.open=function(url,target,features){
    const w=nativeOpen(url,target,features);
    if(url==='' && /width=(900|1000)/.test(features||'')){
      const originalWrite=w?.document?.write?.bind(w.document);
      if(originalWrite)w.document.write=function(html){
        html=String(html)
          .replace('@page{margin:0}','@page{size:78mm 36mm landscape;margin:0}')
          .replace('body{margin:0;font-family:Arial}','body{margin:0;padding:0;width:78mm;height:36mm;font-family:Arial}')
          .replace('.sheet{display:flex;gap:2mm;flex-wrap:wrap}', '.sheet{width:78mm;height:36mm;display:flex;flex-direction:row;flex-wrap:nowrap;gap:2mm;padding:0 2mm;box-sizing:border-box}')
          .replace('.label{width:36mm;height:36mm;padding:2mm;box-sizing:border-box;', '.label{flex:0 0 36mm;width:36mm;height:36mm;padding:2mm;box-sizing:border-box;');
        originalWrite(html);
      };
    }
    return w;
