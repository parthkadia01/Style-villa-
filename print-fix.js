(function(){
  ...
  window.printLabels=function(no,items,bill,customer,source){
    ...
    let h='<!doctype html><html><head><meta charset="utf-8"><title>Fashion Factory Labels</title><style>'+
      '@page{size:78mm 36mm;margin:0}'+
      '*{box-sizing:border-box}'+
      'html,body{margin:0;padding:0;width:78mm;background:#fff;font-family:Arial,sans-serif}'+
      '.row{width:78mm;height:36mm;display:grid;grid-template-columns:36mm 36mm;column-gap:2mm;padding:0 2mm;page-break-after:always;break-after:page}'+
      '.label{width:36mm;height:36mm;padding:1.6mm 1.5mm 1.2mm;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between}'+
    ...
    rows.forEach(({it,s,i,detail})=>{
      ...
      h+='<div class="label">'+
        ...
        '<div class="txt"><b>Bill:</b> '+esc(bill||'—')+'<br><b>Customer:</b> '+esc(customer||'—')+'<br><b>Delivery:</b> '+esc(s.delivery_date||'—')+'<br><b>Qty:</b> '+esc(s.quantity||1)+'</div>'+
        '<svg class="barcode" id="b'+i+'"></svg>'+
        '<div class="code">'+esc(it.bag_barcode||'')+'</div>'+ 
        '</div>';
      h+='<div class="label">'+
        '<div class="brand">FASHION FACTORY</div>'+
        '<div class="txt"><b>Bill:</b> '+esc(bill||'—')+'<br><b>Delivery:</b> '+esc(s.delivery_date||'—')+'</div>'+
        '<div class="detail">'+esc(detail)+'</div>'+
        '<svg class="barcode" id="p'+i+'"></svg>'+ 
        '<div class="code">'+esc(it.product_barcode||'')+'</div>'+ 
        '</div>';
      h+='</div>';
    });
